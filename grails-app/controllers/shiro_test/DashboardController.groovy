    ...
    def index() { 
		...
		if(costcenterstuff2.size() == 0){
			...
			for(def k=1; k < 7; k++){
				...
				weekArray.add([now - k, 1.9, monthBudgetForWeekGraph/daysInMonthForWeekGraph])
...
totalAnnualPower: totalAnnualPower, totalAnnualCarbon: totalAnnualCarbon, weeklyBudgetData: weeklyBudgetData, monthlyBudgetData: monthlyBudgetData, annualBudgetData: annualBudgetData, weekresults: weekArray]
