		...
		if(costcenterstuff2.size() == 0){
			...
			for(def i=timestamps.size() - 1; i > 0; i--){
				...
				if(newDate > yesterday){
					dailyGraphData.add([newDate, powerratings[i], monthBudgetForDailyGraph/timestamps.size()])
					totalDailyPowerCost += powerratings[i];
					totalDailyPower += allPowerConsumptionEntries[i]
					if (alertCounter < 10 && powerratings[i] > monthBudgetForDailyGraph/timestamps.size()){
						if (powerratings[i] == null){
							powerratings[i] = 0.0
						}
						dailyAlertsData.add([newDate, (double)Math.round((powerratings[i] - monthBudgetForDailyGraph/timestamps.size()) * 10000000)/10000000])
					}
			...
			for(def k=7; k >= 0; k--){
				...
				if (weekresults.dailyTotalCents[0] != null) {
					weeklyPowerCounter += weekresults.dailyTotalCents[0] * costCenterCentsPerKw
				}
				...
				cumulativeWeekArray.add([now-k, weeklyPowerCounter, weeklyBudgetCounter])
...
totalAnnualPower: totalAnnualPower, totalAnnualCarbon: totalAnnualCarbon, weeklyBudgetData: cumulativeWeekArray, monthlyBudgetData: cumulativeMonthArray, annualBudgetData: cumulativeYearArray, weekresults: weekArray, dailyAlertsData: dailyAlertsData, alertslength: dailyAlertsData.size()]
